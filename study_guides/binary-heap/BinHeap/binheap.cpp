...
void BinHeap :: extractMax(){
    // gets max in binary heap
    int temp = A[1];
    A[1] = A[size];
    cout << "new index 1 is " << to_string(A[1]) << endl;
    A[size] = temp;
    cout << "deleted at size is " << to_string(A[size]) << endl;
    
    size = size - 1;
    cout << "size is now " << to_string(size) << endl;
    cout << print() << endl;
...
void BinHeap :: heapify(int i){
    ...
    if (A[i] >= max(A[2*i+1], A[2*i])){
        // already heap
        return;
    }
    
    int i_child = 2*i;
    if (A[2*i]<A[2*i+1] && 2*i <= size && 2*i+1 <= size){
        i_child = 2*i+1;
    }
